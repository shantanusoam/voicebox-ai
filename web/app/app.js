...
import { RealtimeVoice } from '/app/realtime.js';
...
const state = {user:null,summary:{},calls:[],appointments:[],tasks:[],devices:[],events:[],view:'overview',activeCall:null,actions:[],busy:false,query:'',callFilter:'all',pipeline:'browser',date:null,slots:[],echo:null,recorder:null,recording:false,mediaStream:null,lastElapsed:null,live:{status:'idle',lines:[],tools:[],level:0,speaking:false,error:'',model:''}};
...
function liveCallPanel(){
  const rt=state.live, on=rt.status!=='idle', ready=rt.status==='live';
  if(!state.user.realtime_available){
    return `<section class="card live-call"><div class="card-header"><div><h3>Live call</h3><p>Full-duplex voice with barge-in.</p></div><span class="pill neutral">NOT CONFIGURED</span></div><div class="card-pad"><p class="small muted">Set <code>OPENAI_API_KEY</code> on the server to enable realtime voice. OpenRouter cannot proxy the Realtime API, so a separate OpenAI key is required.</p></div></section>`;
  }
  const bars=[...Array(14)].map((_,i)=>`<i class="${rt.level*14>i?'on':''}"></i>`).join('');
  return `<section class="card live-call">
    <div class="card-header"><div><h3>Live call</h3><p>Speak naturally. Interrupt any time - it stops talking.</p></div>
      <span class="pill ${ready?'success':on?'warning':'neutral'}">${ready?(rt.speaking?'ASSISTANT SPEAKING':'LISTENING'):on?'CONNECTING':'READY'}</span></div>
    <div class="card-pad">
      <div class="live-row">
        <button class="button ${on?'danger':'primary'}" data-action="live-toggle">${on?'End live call':'Start live call'} ${on?'':'&rarr;'}</button>
        <button class="button secondary" data-action="live-interrupt" ${ready?'':'disabled'}>Interrupt</button>
        <div class="level-meter" aria-hidden="true">${bars}</div>
      </div>
      <div class="live-transcript" id="live-transcript" aria-live="polite">${
        rt.lines.length?rt.lines.slice(-8).map(l=>`<p class="${l.who}"><strong>${l.who==='caller'?'You':'CallBox'}</strong> ${esc(l.text)}</p>`).join(''):'<p class="muted">The live transcript appears here. Nothing is sent until you press Start.</p>'}</div>
      ${rt.tools.length?`<div class="live-tools">${rt.tools.slice(-4).map(t=>`<span class="pill ${t.ok?'success':'warning'}"><code>${esc(t.tool)}</code></span>`).join('')}</div>`:''}
      ${rt.error?`<p class="inline-error">${esc(rt.error)}</p>`:''}
      <div class="hint">${esc(rt.model||'')} &middot; Real microphone audio goes to OpenAI. Facts and bookings still come from this server's tools, not the model.</div>
    </div></section>`;
}

function renderLive(){ if(state.view==='playground'){const el=$('.live-call');if(el)el.outerHTML=liveCallPanel();} }

let voice=null;
function liveLine(who,text){ state.live.lines.push({who,text}); renderLive(); }
async function toggleLiveCall(){
  if(state.live.status!=='idle'){ if(voice)voice.stop(); return; }
  state.live={...state.live,status:'connecting',lines:[],tools:[],error:'',level:0,speaking:false};
  renderLive();
  voice=new RealtimeVoice({
    ready:(e)=>{state.live.status='live';state.live.model=e.model;renderLive();},
    caller:(t)=>liveLine('caller',t),
    assistant:(t)=>liveLine('assistant',t),
    tool:(e)=>{state.live.tools.push({tool:e.tool,ok:!e.result?.error});
               if(e.result&&e.result.committed)refresh();renderLive();},
    level:(v)=>{state.live.level=v;const m=$('.level-meter');
                if(m)[...m.children].forEach((b,i)=>b.classList.toggle('on',v*14>i));},
    speaking:(v)=>{if(state.live.speaking!==v){state.live.speaking=v;renderLive();}},
    error:(m)=>{state.live.error=m;renderLive();toast(m,true);},
    stopped:()=>{state.live.status='idle';state.live.speaking=false;renderLive();refresh();},
  });
  await voice.start();
}
...
function playground(){
 ...
 const providerName=({openrouter:'OpenRouter',openai:'OpenAI'})[state.user.provider_kind]||'Provider';
...
async function actions(event){
 ...
 else if(action==='live-toggle')await toggleLiveCall();
 else if(action==='live-interrupt'&&voice)voice.interrupt();
